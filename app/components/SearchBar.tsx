        ...
        () =>
            function ControlSubstitute({ children, ...props }: ControlProps<SearchElement, false>) {
                const clearHandler = useCallback(
                    (e: React.MouseEvent<HTMLButtonElement, globalThis.MouseEvent> | React.TouchEvent<HTMLButtonElement>) => {
                        e.preventDefault();
                        e.stopPropagation();
                        setSearch('');
            ...
            <AsyncSelect
                ...
                styles={{
                    control: (style, state) => ({ 
                        ...
                        borderColor: state.isFocused ? 'rgba(0, 168, 107, 0.3)' : 'transparent',
                        borderRadius: 'var(--radius-full)',
                        borderWidth: '4px',
                        boxShadow: 'inset 0 1px 2px rgba(0, 0, 0, 0.3)',
                        transition: 'border-color 0.2s ease',
                        '&:hover': {
                            borderColor: state.isFocused ? 'rgba(0, 168, 107, 0.3)' : 'rgba(0, 168, 107, 0.1)',
                            boxShadow: 'inset 0 1px 2px rgba(0, 0, 0, 0.4)'
                        }
                    }),
                    input: style => ({ ...style, width: '100%', color: 'var(--text-primary)' }),
                    placeholder: style => ({ ...style, pointerEvents: 'none', color: 'var(--text-secondary)' }),
                    menu: style => ({ 
                        ...style, 
                    ...
                    option: (style, state) => ({
                        ...style,
                        backgroundColor: state.isSelected ? 'var(--primary-500)' : state.isFocused ? 'var(--bg-tertiary)' : 'transparent',
...
function KeyIndicator() {
    return <div className="search-refined-key">/</div>;
}
...
function ClearIndicator({
    ...
    return (
        <button className="search-refined-clear" onClick={onClick} onTouchStart={onTouchStart}>
            <X size={16} />
        </button>
    );
}
