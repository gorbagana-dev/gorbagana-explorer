...
export function lamportsToGor(lamports: number | bigint): number {
    if (typeof lamports === 'number') {
        return lamports / LAMPORTS_PER_GOR;
    }
    ...
    const lamportsString = absLamports.toString(10).padStart(10, '0');
    const splitIndex = lamportsString.length - 9;
    const gorString = lamportsString.slice(0, splitIndex) + '.' + lamportsString.slice(splitIndex);
    return signMultiplier * parseFloat(gorString);
}

export function lamportsToGorString(lamports: number | bigint, maximumFractionDigits = 9): string {
    const gor = lamportsToGor(lamports);
    return new Intl.NumberFormat('en-US', { maximumFractionDigits }).format(gor);
}
